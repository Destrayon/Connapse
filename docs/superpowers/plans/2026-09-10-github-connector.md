...
8. **Conditional-request probes.** Goal: an idle repository costs no primary budget. Done when: fixed-parameter probes on issues, issue comments, and review comments gate the `since` fetch on an ETag change, with tests showing an idle cycle spends only 304s.
   **Phase 8 status — CODE COMPLETE (2026-09-23).** With credentials, every check is conditional (`GitHubApiClient.GetIfChangedAsync`, `If-None-Match`; an authenticated 304 is free): the visibility check (`GitHubRepositoryGuard` now takes and returns an ETag — kept in `state.json` for issues, in `connapse-visibility.etag` beside the docs mirror) and three newest-first, one-per-page probes over issues, issue comments, and review comments. A sweep runs only when its probe changed (or its mark is null, or comments are owed); ETags are committed only after the sweeps they gate finish, so a budget-interrupted cycle is never masked by a stale 304. The comment probes replace the hourly comment sweep when authenticated (it remains for credential-less test fixtures). Idle issues source: 4 requests a cycle, all 304. Docs are fetched with git every cycle as before (no REST cost). Verified: unit suite green incl. idle-is-all-304s, comment-edit-next-cycle, interrupted-sweep-not-masked, and private-after-settling-still-refused; GitHub integration green.
...
9. **Add public repositories on an App connection.** Goal: an admin adds a public repository from New source by choosing a GitHub App connection. Done when: the dialog accepts a public repository address, refuses a private one with a message pointing at Phase 10, creates the docs and issues sources on that connection, and existing connection-less GitHub sources have a documented migration or are refused with a clear message.
   **Phase 9 status — CODE COMPLETE (2026-09-23).** Choosing a GitHub connection in New source shows repository fields (address or `owner/repo`, docs/issues/comments, doc patterns) in place of name, description, and interval, with a preview of the two source names. Create asks GitHub — as that installation, pinned (`GitHubRepositoryLookup`) — whether the repository exists and is public; private and internal ones are refused with a pointer to Phase 10. Sources are created on the connection with no special interval (idle polling is free since Phase 8). `GitHubRepositoryForm` is back from `c8ebe95`, now building connection-bound requests. Existing connection-less GitHub sources are refused by the factory with a message naming the fix (Phase 7) and shown by provider on the Sources page; no automatic migration, since none exist outside tests. Verified: unit suite green (+19 form, +4 lookup, +2 dialog markup); GitHub and sources-endpoint integration green.

   **Phase 10 — BLOCKED on a live GitHub App (2026-09-23).** The permission check's design rests on the collaborator-permission endpoint, and whether an installation token with only `metadata:read` may call it (GitHub's docs contradict each other) decides the App's permissions and the resolver's shape. A security filter whose central call has not been seen to work should not be built on inference. Unblocks when an App exists (Phases 6–9 make that one click): run the check against a private repository, then build.
...
10. **Private repositories with per-user permission filtering.** Goal: private repositories can be added, and search shows their documents only to users GitHub says can read them. Done when: the deferred design from the 2026-09-09 research is built — user-to-server identity linking, live per-repository permission checks at query time, fail-closed — and verified by tests *before* the dialog offers private repositories; the open `metadata:read` vs push question for the collaborator-permission endpoint is settled by a live test.
   ...
   **Consequence for App ownership (research 2026-09-23).** A private App can be authorized only by members of its owning account, so a private App on a personal account lets nobody but its owner sign in — Phase 10 would be unusable. A private org-owned App covers members of that org (outside collaborators probably cannot sign in — unverified); a public App covers everyone. Phase 6's Easy flow must therefore check that a named org exists before posting the manifest, and make the App public when no org is named. *Built on #514 (2026-09-23):* a Continue step asks GitHub (`GET /users/{login}`, anonymous) whether the name is an organisation; the pool (#515) borrows only installations an admin added as a connection, so strangers installing a public App gain nothing.
   **Revised (user, 2026-09-23): the provider App is always public, and it is the bare minimum for everything GitHub.** "Public" governs who may install the App, not which repositories it reads — installed on an org with `contents:read`, it reads that org's private repositories. One App installed on every org is how one Connapse covers several orgs (a private App installs only on its owner). The Providers card now leads with one button that creates the App on the admin's own account; "Have an organisation own the App instead" is an opt-in with the existence check. Per-org private connector Apps are a possible later extra, not built. **Gates:** no GitHub connection can be created until the provider App exists (the Connections page disables the option and re-checks the installation at save), and in Phase 10 **no user can link their GitHub account until the provider App exists** — user sign-in goes through the same App. Sourcegraph (the closest analogue) offers optional org + public/private, private by default.
