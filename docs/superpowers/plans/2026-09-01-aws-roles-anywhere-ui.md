...
- **Codex adversarial review** (verdict: needs-attention / "do not ship" against the first cut). Four findings, all addressed at the core:
  - **[high] false "Ambient" from a probe** → FIXED fully. Mode no longer inferred from probe success (a resolving default chain is env keys / shared file / web identity just as much as an instance role). Extracted `AwsAccessCard.ResolveMode(requirementsLoaded, hasStoredRolesAnywhere)` → Loading / RolesAnywhere / NotStored; RA setup stays offered whenever nothing is stored (an off-AWS box on bootstrap keys is no longer steered away from RA); a resolving probe is only an informational note ("credentials from its environment"), never an instance-role claim; load-transient gated to Loading. Unit-tested (4 cases in `AwsAccessCardTests`).
  - **[high] reset forgets but does not revoke** → FIXED (honest). Relabelled "Remove stored credential", confirm label "Remove — AWS access stays live", help + post-reset alert state outright that the trust anchor/profile stay enabled and the cert keeps working until expiry or cleanup. Removes the dangerous "reported clean while still live" gap. **Deferred:** automated remote revoke (needs an AWS RolesAnywhere write client + `rolesanywhere:DeleteTrustAnchor`, which the read-only identity lacks).
  - **[medium] intermediate-CA certs can't authenticate** → FIXED (honest scope). Manual-values UI states the cert must be the trust anchor or a leaf issued directly by it; intermediate chains unsupported. **Deferred:** full `X-Amz-X509-Chain` support end-to-end.
  - **[medium] circuit-loss strands resources** → FIXED (warning). "Keep this page open until you paste back" note; abandoned runs leave a harmless fingerprint-named orphan a fresh run replaces. **Deferred:** persisted encrypted setup draft. Pushed back on deterministic key zeroization — managed-string PEMs are how the whole credential store + SFTP keys already work.
- **Verified after fixes:** build clean; full unit suite green (Core 1120, Storage 96, Web 115, +4 resolver tests).
- **Next:** Push + PR against the epic branch. Three deferred enhancements tracked for follow-up (remote-revoke, intermediate-chain, persisted-draft).
