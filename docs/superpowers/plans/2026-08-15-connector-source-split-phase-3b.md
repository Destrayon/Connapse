...
Still not covered, and worth knowing before merge: the migration has only ever been applied to an empty `chunks` table, so the upgrade path on populated tables rests on `NOT VALID` skipping the scan rather than on a test. The delta path has no production implementer yet, so fakes are the only thing to test it against.

Verified: `dotnet test` — 1062 passed, 0 failed.
