...
{
    /// <summary>
    /// How often the issues source is polled. Each idle cycle costs one request from GitHub's
    /// anonymous budget of 60 an hour, shared by every source on this host; every 15 minutes
    /// leaves room for a dozen repositories. Docs are fetched over git, outside that budget, so
    /// they keep the default.
    /// </summary>
    public const int IssuesSyncIntervalSeconds = 900;
    ...
    /// <summary>The sources to create, docs first. Call <see cref="Validate"/> first.</summary>
    public IReadOnlyList<CreateSourceRequest> ToRequests()
    {
        ...
        if (IncludeIssues)
        {
            ...
            requests.Add(new CreateSourceRequest(
                ...
                SyncIntervalSeconds: IssuesSyncIntervalSeconds,
                Provider: ConnectionProvider.GitHub));
        }
